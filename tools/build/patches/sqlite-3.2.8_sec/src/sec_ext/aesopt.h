...
/*  1. VIA ACE SUPPORT
    ...
    assembler instructions and is only implemented for Microsoft VC++ and GCC
    on the x86.  If VIA ACE is known to be present the define VIA_ACE_PRESENT 
	will remove the ordinary code for encryption/decryption. If VIA_ACE_SUPPORT
    is defined without VIA_ACE_PRESENT this code will be present but VIA will be
    used if it is detected (both present and enabled).  If VIA ACE is to be used
    all AES encryption contexts MUST be aligned on 16 byte boundaries. The other
	input/output buffers do not need to be 16 byte aligned but there are very
	large performance gains if this can be arranged.
*/

#if 0
#define VIA_ACE_PRESENT
#define VIA_ACE_SUPPORT
#endif

#if 0 && !defined( VIA_ACE_SUPPORT ) && (defined( _MSC_VER ) || defined( __GNUC__ ))
#define VIA_ACE_SUPPORT
#endif
