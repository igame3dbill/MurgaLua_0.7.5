...
#define AES_1_BLOCK /* define for single block interface            */
...
#else
aes_rval aes_ecb_encrypt(const unsigned char *in, unsigned char *out, int no_block, const aes_encrypt_ctx cx[1]);
#define aes_encrypt(in,out,cx)  aes_ecb_encrypt((in), (out), 1, (cx))
#endif
...
#else
aes_rval aes_ecb_decrypt(const unsigned char *in, unsigned char *out, int no_block, const aes_decrypt_ctx cx[1]);
#define aes_decrypt(in,out,cx)  aes_ecb_decrypt((in), (out), 1, (cx))
#endif
